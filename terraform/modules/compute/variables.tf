...
variable "waf_login_rate_limit" {
  description = "Max requests per IP per 5-minute window on /login before AWS WAF blocks it. AWS's own minimum allowed value is 10."
  type        = number
  default     = 10
}
variable "waf_buy_rate_limit" {
  description = "Max requests per IP per 5-minute window on /buy before AWS WAF blocks it. AWS's own minimum allowed value is 10. This is the module brief's live-demo threshold -- deliberately low so hammering past it is fast."
  type        = number
  default     = 10
}